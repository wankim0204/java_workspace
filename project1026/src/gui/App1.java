...
class App1{
	public static void main(String[] args){
		...
		대응법1: "사용하려는 클래스가 대충 어떤 목적으로 지원되는것인지 그 용도파악"
		대응법2: "자바의 모든 객체는 결국 3가지 유형밖에 없다"
		대응법3: "클래스는 쓰라고 만든것이다, 따라서 메모리에 올리는 법을 알면된다"

			1) 일반클래스 : new 하면 된다!! new 뒤의 생성자조사(api문서를 통해)
			2) 추상클래스  : new 불가하므로, 자식을 정의해서 new하거나 이미 구현한 인스턴
								스를 이용하면 된다.(api 문서 통해)
			3) 인터페이스 : new  불가하므로, 자식을 정의해서 new하거나 이미 구현한 인스턴
			                   스를 이용하면 된다.(api 문서 통해)
		*/
		...
		Image img = kit.getImage("D:/workspace/java_workspace/project1026/res/1.png");
		...
		//언어에서는 랭더링(직접 그리는 작업)을 해야 하기 때문이다..
