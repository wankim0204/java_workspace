...
class LoginForm extends Frame{
	...
	public LoginForm(){
		...
		//this.setLayout(new BorderLayout());//필요 없다!! 왜 디폴트가 이미 보더레이아웃이므로..
		...
		//공유가 가능하며, public으로 선언하여, 모든 객체가 접근할 수 있또록 접근제한을 풀어놓은 데이터
		...
		p_center.add(la_id);//id 라벨 부착 1행1열
		p_center.add(t_id);//id 텍스트박스 부착1행2열
		p_center.add(la_pass);//pass 라벨부착2행 1열
		p_center.add(t_pass); //pass 텍스트박스 부착 2행 2열

		//Panel은 아무런 배치관리자를 적용하지 않으면 대폴트가 FlowLayout이다!!
