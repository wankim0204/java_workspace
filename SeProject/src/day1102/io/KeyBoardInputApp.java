...
즉 실행중인 프로그램으로 데이터가 흘러들어온다며, 이 모드것이 다 입력스트림이다!!
따라서 이 예제에서는 파일을 대상으로 데이터를 읽는 것이 아니라, 
키보드를 대상으로 데이터를 읽어와 보자!!
주의)  파일과는 달리, 키보드의 스트림은 자바에서 생성할수 있는 것이 아니라, 이미 OS 차원에서 스트림이 
...
public class KeyBoardInputApp {
	...
	public static void main(String[] args) {
		...
		try { // shift + alt + z
			while(true) {
				data = reader.read(); //1byte읽기, read() 의 특징, 입력이 완료되지 않으면 무한대기상태로 머물러 있음
