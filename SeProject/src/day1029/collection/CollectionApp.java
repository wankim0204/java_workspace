 ...
 * Collection Framework(전제 조건은 객체만을 대상으로 한다.) 
  - 자바언어에서는 객체를 모아서 처리할때 유용한 api를 지원하는데, 
  	이  api 를 가리켜 컬렉션 프레임웍이라 한다, 그리고  java.util 에서 지원한다
  - 컬렉션 프레임웍에서 지원하는 객체의 수는 상당히 많기는 하지만,크게는 모여진 모습에 따라서 
  (1)순서있는 유형 List형: 배열과 거의 같다!!  [][][][]
  								자바의 배열과 차이가 있다면,
  								배열: 1.반드시 생성할때 크기를 명시해야 한다. 따라서 동적으로 늘어날 수 없다(고정적)
  								       2.자료형을 섞어 사용할 수 없다. 
...
public class CollectionApp {
	...
	public void showList() {
		...
		//js 의 배열과 동일)  동작방식
		list.add(new JButton("첫버튼"));//0
		//list.add("사과"); //1
		//list.add("복숭아");
		//list.add("멜론");
		list.add(new JButton("마지막버튼"));
		System.out.println("리스트의 데이터 수는 "+list.size()); //배열은 length 지만, 컬렉션 프레임웍은 모두 size()이용
		
		
		for(int i=0;i<list.size();i++) {
			JButton bt1=list.get(i); //섞어서 들어가므로, 반환형은 예측할수 없기 때문에 Object 형이 반환된다..
	...
	public void showSet() {
		...
		while(it.hasNext()) {//요소가 존재하는지 판단, true인동안 while문 동작!!
	...
	public void showMap() {
		...
		Set<String> keySet = map.keySet(); //key 만 따로 추출한다!!
