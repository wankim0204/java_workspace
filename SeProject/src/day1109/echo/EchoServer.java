/*
 * 최종 목표는 채팅(멀티케스팅 구현)이지만, 일단 기초인 Echo System을 먼저 학습한다
 * */
...
public class EchoServer {
	...
	public EchoServer() {
		...
		try {
			...
			//클라이언트가 보낸 메시지 받기!!(메시지를 받는 것은 실행중인 프로그램으로 데이터가 들어오는것이므로
			//입력스트림으로 처리해야 한다..)
			//소켓으로부터 스트림을 뽑아낼 수 있다.
			InputStream is = socket.getInputStream(); //바이트기반의 입력스트림(한글깨짐, 영문으로하자)
			int data;
			while(true) {
